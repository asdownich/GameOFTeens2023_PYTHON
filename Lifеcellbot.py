...
@dp.message_handler(text=[tariff for tariff in gadjet_descriptions.keys()])
async def gadjet_description(message: types.Message):
    tariff = message.text
    description = gadjet_descriptions.get(tariff)
    await message.answer(f'{tariff}: {description}')

#family_types
@dp.message_handler(text='Смарт сім\'я')
async def gadjet_types(message: types.Message):
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(*family_buttons)
    await message.answer('Виберіть тип тарифу:', reply_markup=keyboard)

@dp.message_handler(text=[tariff for tariff in family_descriptions.keys()])
async def gadjet_description(message: types.Message):
    tariff = message.text
    description = family_descriptions.get(tariff)
    await message.answer(f'{tariff}: {description}')
#gadjet_types
